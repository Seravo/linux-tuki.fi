...
class Breadcrumb_Trail {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				if ( 1 === $item_position && 1 < $item_count ) {
					$item_class .= ' breadcrumbs__item--start';
			...
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="breadcrumbs__container" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
	...
	protected function add_paged_items() {
		...
		elseif ( is_paged() && true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['paged'],
	...
	protected function add_week_archive_items() {
		...
		// Add the week item.
		if ( is_paged() ) {
			$this->items[] = esc_url( get_archives_link( add_query_arg( [
				'm' => get_the_time( 'Y' ),
				'w' => get_the_time( 'W' )
			], home_url() ), $week, false ) );
		} elseif ( true === $this->args['show_title'] ) {
	...
	protected function add_post_parents( $post_id ) {
		...
		while ( $post_id ) {
			...
			$parents[] = sprintf( '<a href="%s">%s</a>', esc_url( get_permalink( $post_id ) ),
				get_the_title( $post_id ) );
	...
	protected function add_post_terms( $post_id, $taxonomy ) {
		...
		if ( $terms && ! is_wp_error( $terms ) ) {
			...
			$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_term_link( $term, $taxonomy ) ),
				$term->name );
		}
	}
