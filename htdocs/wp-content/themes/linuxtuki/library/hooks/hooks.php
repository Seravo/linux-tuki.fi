...
add_filter( 'tiny_mce_before_init', function ( $init ) {
	...
	$init['block_formats']           = "Paragraph=p;Address=address;Pre=pre;Heading 1=h1;Heading 2=h2;Heading 3=h3;Heading 4=h4";
 ...
 *
 * @hook linux_tuki_after_body
 */
add_action( 'linux_tuki_after_body', function () {
	$options = get_option( 'linux_tuki_general_options' );

	if ( ! empty( $options['linux_tuki_tagmanager_body'] ) ) :
		echo $options['linux_tuki_tagmanager_body'];
	endif;
} );
...
add_action( 'wp_head', function () {
	...
	if ( ! empty( $options['linux_tuki_tagmanager_head'] ) ) :
		echo $options['linux_tuki_tagmanager_head'];
	endif;
